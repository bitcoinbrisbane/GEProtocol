...
import DrawerLiquidityV3 from "./DrawerLiquidityV3";
...
const TableLiquidityV3: React.FC = () => {
      ...
      setData([
        {
          pool: "ETH - USDT",
          coins: [
            { name: "ETH", image: "/coin/ETH.svg" },
            { name: "USDT", image: "/coin/USDT.svg" },
          ],
          apr: "12%",
          volume: "$3000",
          fees: "$150",
  ...
  const totalVolume = data.reduce(
    (acc, item) =>
      acc + parseFloat(item.volume.replace("$", "").replace(",", "")),
    0
  );
  const totalFees = data.reduce(
    (acc, item) =>
      acc + parseFloat(item.fees.replace("$", "").replace(",", "")),
    0
  );
                ...
                {paginatedData.map((item, index) => (
                  <tr key={index} className="text-[12px] md:text-md lg:text-lg">
                    ...
                    <td className="py-4 px-2 text-center">
                      <Link
                        href={`/liquidity/concentrated-pools/deposit?coin=${item.balance[0].name}-${item.balance[1].name}`}
...
export default TableLiquidityV3;
